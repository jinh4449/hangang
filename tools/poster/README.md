...
| 약침 | 효과 | 이럴 때 |
|---|---|---|
| 소염약침 | 염증 가라앉힘 · 부기 완화 | 삐끗한 통증 · 부은 관절 |
| 통증약침 | 통증 완화 · 뭉친 근육 풀기 | 목·허리 통증 · 결림 |
| 봉약침 | 염증 완화 · 면역력 관리 | 무릎 관절염 · 오십견 |
| DNA약침(PDRN) | 다친 부위 회복 | 인대·힘줄 손상 · 어깨 통증 |
| 태반약침 | 기력 회복 · 면역력 관리 | 만성 피로 · 갱년기 증상 |

묶음 머리에는 `1회 금액 · 10회 금액` 을 함께 적고, 10회 쪽에만 형광펜 자국을 넣어
이벤트가임을 표시한다. 10회 금액은 정가 × 10 × 0.7 로 계산한 값이다.
