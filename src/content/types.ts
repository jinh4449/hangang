...
export type Symptom = {
  ...
  care: {
    title: string;
    /**
     * 검색 결과에 뜨는 설명. 화면 맨 위에도 이 글이 서지만, 검색 결과는
     * 160자까지 보여 주므로 사실을 밀도 있게 담는다.
     */
    lede: string;
    /**
     * 화면 맨 위에만 뜨는 한 줄. 없으면 lede 를 쓴다.
     *
     * 읽는 자리가 다르면 필요한 길이도 다르다. 검색 결과는 클릭 전에 읽는
     * 글이라 길어도 되지만, 페이지를 연 사람은 이미 들어왔으므로 첫 줄이
     * 길면 읽지 않고 넘긴다.
     */
    leadLine?: string;
    /** 이 증상에서 부위별 안내로 갈라지는 곳. 통증처럼 부위가 갈리는 데만 쓴다 */
    partSlugs?: string[];
    /** "이런 증상이 있다면" — 자기 확인용 */
    signs: string[];
